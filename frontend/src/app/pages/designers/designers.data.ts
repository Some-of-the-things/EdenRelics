...
export const DESIGNERS: DesignerProfile[] = [
  {
    ...
    history: [
      'Leslie Fay was a US women\'s apparel manufacturer founded in 1947 by Fred Pomerantz and named after his daughter. It produced moderately priced women\'s clothing — mainly dresses and sportswear, alongside careerwear and occasionwear — sold through department and specialty stores across the United States. It functioned primarily as a mass-market supplier, not a high-fashion design house, and was widely described as producing "stylishly conservative" womenswear for the middle market.',
      'Its strongest commercial period was the 1980s to the early 1990s. At its peak around 1990 revenues reached roughly $859 million, making it a major player in US department-store womenswear, and it expanded its branding through multiple labels and licensing deals during this period.',
      'In 1993 the company disclosed major accounting irregularities and filed for Chapter 11 shortly after, bringing store-order reductions, plant closures and layoffs. It later restructured and continued in reduced form through the late 1990s and early 2000s. For collectors this makes the peak-era pieces, broadly pre-mid-1990s, the ones most associated with the label.',
    ],
    identificationHeading: 'How to recognise vintage Leslie Fay',
    identification: [
      'Start with the woven "Leslie Fay" label. Its design changed across the decades and the company\'s many sub-labels, so it confirms the brand but no single version pins a piece to an exact year on its own.',
      'Expect era-typical fabrics, not a signature cloth. Peak-era Leslie Fay used polyester and other synthetic blends common for the period — so fabric helps place the era but does not authenticate the brand, and no one fibre is a mark of a "genuine" piece.',
      'Read the construction and prints as era context. Mass-produced, structured, office-appropriate tailoring to department-store standards, and seasonal prints — florals, geometrics and novelty patterns — are typical of the period, but they are shared across many department-store brands, so they date a piece rather than prove the label.',
      'Recognise it by triangulation, not a signature. Because Leslie Fay was a mass-market house, no single print or silhouette — not polka dots, not peplums — is brand-defining; read the label, the construction and the overall 1980s–90s department-store styling together rather than relying on any one "tell".',
    ],
