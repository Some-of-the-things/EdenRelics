...
public class SeoTests : IClassFixture<ApiFactory>
{
    ...
    public async Task GetKeywords_AsAdmin_ReturnsEmptyList()
    {
        ...
        List<KeywordResponse>? keywords = await client.GetFromJsonAsync<List<KeywordResponse>>("/api/seo/keywords", JsonOptions);
    ...
    public async Task AddKeyword_AsAdmin_ReturnsCreatedKeyword()
    {
        ...
        HttpResponseMessage response = await client.PostAsJsonAsync("/api/seo/keywords", new
        {
        ...
        KeywordResponse? keyword = await response.Content.ReadFromJsonAsync<KeywordResponse>(JsonOptions);
    ...
    public async Task UpdateKeyword_AsAdmin_ModifiesKeyword()
    {
        ...
        HttpResponseMessage createResponse = await client.PostAsJsonAsync("/api/seo/keywords", new
        {
            keyword = "original keyword",
            pageUrl = "https://edenrelics.co.uk",
            position = 20
        });
        KeywordResponse? created = await createResponse.Content.ReadFromJsonAsync<KeywordResponse>(JsonOptions);

        HttpResponseMessage updateResponse = await client.PutAsJsonAsync($"/api/seo/keywords/{created!.Id}", new
        {
        ...
        KeywordResponse? updated = await updateResponse.Content.ReadFromJsonAsync<KeywordResponse>(JsonOptions);
    ...
    public async Task UpdateKeyword_NonExistent_Returns404()
    {
        ...
        HttpResponseMessage response = await client.PutAsJsonAsync($"/api/seo/keywords/{Guid.Empty}", new
        {
            keyword = "nope"
        });
    ...
    public async Task DeleteKeyword_AsAdmin_Returns204()
    {
        ...
        HttpResponseMessage response = await client.DeleteAsync($"/api/seo/keywords/{created!.Id}");
    ...
    public async Task DeleteKeyword_NonExistent_Returns404()
    {
        ...
        HttpResponseMessage response = await client.DeleteAsync($"/api/seo/keywords/{Guid.Empty}");
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
    ...
    public async Task Analyse_MissingUrl_Returns400()
    {
        ...
        HttpResponseMessage response = await client.PostAsJsonAsync("/api/seo/analyse", new
        {
            url = ""
        });
