...
public class ProductsController : ControllerBase
{
    ...
    public async Task<ActionResult<ProductDto>> Create(CreateProductDto dto)
    {
        ...
        // Must include soft-deleted rows: Products.Sku / Products.Slug have
        // partial unique indexes that don't filter on IsDeleted, so a deleted
        // row still blocks a new insert with the same SKU or slug.
        IEnumerable<Product> existing = await _repository.GetAllAsync(includeDeleted: true);
        List<Product> existingList = existing.ToList();

        string baseSlug = SlugHelper.Generate(dto.Name);
        string uniqueSlug = string.IsNullOrEmpty(baseSlug)
            ? ""
            : SlugHelper.MakeUnique(baseSlug, existingList.Select(p => p.Slug));

        string sku;
        if (!string.IsNullOrWhiteSpace(dto.Sku))
        {
            string? validationError = SkuGenerator.Validate(dto.Sku);
            if (validationError is not null)
            {
                return BadRequest(new { error = validationError });
            }
            sku = dto.Sku.Trim();
            if (existingList.Any(p => string.Equals(p.Sku, sku, StringComparison.OrdinalIgnoreCase)))
            {
                return BadRequest(new { error = $"SKU '{sku}' is already in use." });
            }
        }
        else
        {
            sku = SkuGenerator.Next(existingList.Select(p => p.Sku));
        }

        ProductStatus status = dto.Status ?? (dto.InStock ? ProductStatus.Live : ProductStatus.Sold);
        ...
        bool autoGenerated = string.IsNullOrWhiteSpace(dto.Sku);
        try
        {
            await _repository.AddAsync(product);
        }
        catch (DbUpdateException) when (autoGenerated)
        {
            // Race on auto-generated SKU: refetch and retry once with a new number.
            IEnumerable<Product> refetched = await _repository.GetAllAsync(includeDeleted: true);
    ...
    public async Task<ActionResult<ProductDto>> Update(Guid id, UpdateProductDto dto)
    {
        ...
        if (dto.Sku is not null)
        {
            ...
            string trimmedSku = dto.Sku.Trim();
            if (!string.Equals(trimmedSku, product.Sku, StringComparison.OrdinalIgnoreCase))
            {
                IEnumerable<Product> others = await _repository.FindAsync(p => p.Id != id);
                if (others.Any(p => string.Equals(p.Sku, trimmedSku, StringComparison.OrdinalIgnoreCase)))
                {
                    return BadRequest(new { error = $"SKU '{trimmedSku}' is already in use." });
                }
        ...
        if (dto.SalePrice.HasValue)
        {
            ...
            decimal? newSalePrice = dto.SalePrice.Value == 0 ? null : dto.SalePrice.Value;
            product.SalePrice = newSalePrice;
            if (newSalePrice.HasValue && oldSalePrice != newSalePrice)
            {
                product.SalePriceSetAtUtc = DateTime.UtcNow;
            }
            else if (!newSalePrice.HasValue)
            {
