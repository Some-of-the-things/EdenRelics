...
/// <summary>
/// SEO domain logic: health snapshots, on-page analysis, tracked-keyword CRUD, and the
/// GSC/GA4 traffic rollups (in SeoService.Traffic.cs). All persistence goes through
/// repositories; the GSC/GA4 ingest and rank-check services are self-contained.
/// </summary>
public partial class SeoService(
    IRepository<SeoHealthSnapshot> snapshots,
    IRepository<TrackedKeyword> keywords,
    IRepository<SearchConsoleDailyTotal> scTotals,
    IRepository<SearchConsoleDailyQuery> scQueries,
    IRepository<SearchConsoleDailyPage> scPages,
    ...
    GoogleAnalyticsService ga4) : ISeoService
{
    // --- Health snapshots ---

    public async Task<List<SeoHealthSnapshot>> GetHealthSnapshotsAsync(int take)
    {
        int clamped = Math.Clamp(take, 1, 365);
        return await snapshots.Query()
            .OrderByDescending(s => s.TakenAtUtc)
            .Take(clamped)
            .ToListAsync();
    }

    public async Task<SeoHealthSnapshot?> GetLatestHealthSnapshotAsync()
    {
        return await snapshots.Query()
            .OrderByDescending(s => s.TakenAtUtc)
            .FirstOrDefaultAsync();
    ...
    public async Task<SeoHealthSnapshot> RunHealthSnapshotAsync()
    {
        return await seoHealth.CaptureAsync();
    }

    // --- On-page analysis ---

    /// <summary>Fetches and analyses a page's on-page SEO. Returns null if the URL can't be fetched.
    /// The caller is responsible for validating the URL is a safe, public http(s) address first.</summary>
    public async Task<SeoAnalysisResult?> AnalyseAsync(string url)
    {
    ...
    public async Task<List<TrackedKeywordDto>> GetTrackedKeywordsAsync()
    {
        List<TrackedKeyword> rows = await keywords.Query()
            .OrderByDescending(k => k.UpdatedAtUtc)
            .ToListAsync();
        return rows.Select(ToDto).ToList();
    }

    public async Task<TrackedKeywordDto> AddKeywordAsync(CreateTrackedKeywordDto dto)
    {
    ...
    public async Task<List<TrackedKeywordDto>> CheckAllKeywordsAsync()
    {
        await rankChecker.CheckAllKeywordsAsync();
        return await GetTrackedKeywordsAsync();
    }

    public async Task<TrackedKeywordDto?> UpdateKeywordAsync(Guid id, UpdateTrackedKeywordDto dto)
    {
        TrackedKeyword? keyword = await keywords.GetByIdAsync(id);
        if (keyword is null)
        {
            return null;
        }
        ...
        await keywords.UpdateAsync(keyword);
        return ToDto(keyword);
    }
