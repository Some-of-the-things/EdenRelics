...
public class ContentService(IRepository<SiteContent> repository) : IContentService
{
    private static readonly HashSet<string> LocalePrefixes = ["en", "fr", "de", "es", "it", "nl", "pt", "sv", "da", "nb", "ja", "ko"];

    public async Task<Dictionary<string, string>> GetAllAsync(string? locale)
    {
        List<SiteContent> entries = await repository.Query().ToListAsync();
        ...
        return StripLocaleKeys(all);
    }

    public async Task<Dictionary<string, string>> UpdateAllAsync(Dictionary<string, string> content)
    {
        List<SiteContent> existing = await repository.Query().ToListAsync();
        Dictionary<string, SiteContent> byKey = existing.ToDictionary(e => e.Key);

        List<SiteContent> toAdd = [];
        foreach (KeyValuePair<string, string> kv in content)
        {
            ...
            {
                toAdd.Add(new SiteContent { Key = kv.Key, Value = kv.Value });
            }
        }

        // Single save commits both the inserts and the in-place edits above.
        ...
        List<SiteContent> updated = await repository.Query().ToListAsync();
        Dictionary<string, string> result = StripLocaleKeys(updated.ToDictionary(e => e.Key, e => e.Value));
        foreach (KeyValuePair<string, string> kv in Defaults)
        {
    ...
    private static Dictionary<string, string> StripLocaleKeys(Dictionary<string, string> source)
    {
    ...
    private static readonly Dictionary<string, string> Defaults = new()
    {
        ...
        ["policy.returns.updated"] = "July 2026",
        ["policy.returns.content"] = """
            ...
            <div class="policy__section"><h2>3. Conditions for Returns</h2><p>You are welcome to inspect and try an item on as you would in a shop. When you exercise your right to cancel, we ask that it is returned unworn (beyond trying it on), unwashed and, where possible, with any tags still attached. As our pieces are vintage and one-of-a-kind, please handle them with particular care. If an item is returned showing handling beyond what is reasonably necessary to establish its nature, characteristics and condition, we may make a proportionate deduction from your refund to reflect any reduction in its value. This does not affect your statutory right to cancel.</p></div>
            ...
            <div class="policy__section"><h2>8. Faulty or Incorrectly Described Items</h2><p>We take great care to accurately describe and photograph every item. Under the Consumer Rights Act 2015, if an item is faulty or significantly not as described you have a short-term right to reject it for a full refund &mdash; including original and return postage costs &mdash; within 30 days of receipt. After 30 days, and within the first six months, you remain entitled to a repair, replacement or refund; where a repair or replacement is not possible or does not resolve the fault, you may claim a refund. Please contact us as soon as you notice a problem. Minor signs of wear are to be expected with vintage garments and are noted in the item description and condition grading; these are not considered faults.</p></div>
